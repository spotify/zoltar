...
public class JTensorTest {
  ...
  private void testMultidimensionalStringTensor(
      final JTensor jt, final Object[] expectedValue, final long[] expectedDimensions) {
    assertEquals(DataType.STRING, jt.dataType());
  ...
  private byte[][][][] toByteArray(final String[][][] stringValue) {
    return Arrays.stream(stringValue)
        .map(
            subArray1 ->
                Arrays.stream(subArray1)
                    .map(
                        subArray2 ->
                            Arrays.stream(subArray2)
